...
# ══════════════════════════════════════════
# WORK.INK TOKEN VERIFICATION
# ══════════════════════════════════════════
def verify_workink_token(token: str, single_use: bool = True) -> bool:
    """Xác minh token từ Work.ink. single_use=True để xoá token sau khi dùng."""
    try:
        param = "?deleteToken=1" if single_use else ""
        res   = _requests.get(
            f"https://work.ink/_api/v2/token/isValid/{token}{param}",
            headers={"Authorization": f"Bearer {WORKINK_API_KEY}"},
            timeout=5
        )
        data = res.json()
...
def _check_rate_limit(ip: str, max_calls: int = 1, window_seconds: int = None) -> int | None:
    """
    Trả về số giây còn lại nếu bị rate limit, None nếu OK.
    window_seconds mặc định = cooldown_hours của config.
    """
    if window_seconds is None:
        cfg            = _get_cfg()
        window_seconds = cfg.get("cooldown_hours", 24) * 3600
...
WORKINK_API_KEY  = os.getenv("WORKINK_API_KEY")
...
if not WORKINK_API_KEY:
    raise RuntimeError("Thiếu biến môi trường WORKINK_API_KEY")
...
def generate_code(request: Request, token: str = None, hash: str = None):
    """
    Website gọi sau khi user vượt Work.ink.
    Work.ink gửi token qua ?hash=TOKEN hoặc ?token=TOKEN.
    Backend xác minh token với Work.ink API trước khi tạo mã.
    """
    # Chấp nhận cả ?hash= và ?token=
    wink_token = hash or token
    ...
    # Xác minh token với Work.ink (single_use → xoá sau khi dùng)
    valid = verify_workink_token(wink_token, single_use=True)
    if not valid:
        raise HTTPException(status_code=403, detail={
            "error":   "invalid_token",
            "message": "Token không hợp lệ hoặc đã được sử dụng. Vui lòng vượt link lại.",
        })
